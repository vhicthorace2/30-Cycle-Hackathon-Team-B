...
export const workerJobs = pgTable(
  ...
  (table) => ({
    workerJobsParentIdx: index('worker_jobs_parent_job_id_idx').on(
      table.parentJobId,
    ),
    workerJobsQueueStatusIdx: index('worker_jobs_queue_status_idx').on(
      table.queueName,
      table.status,
    ),
    workerJobsRequestIdx: index('worker_jobs_request_id_idx').on(
      table.requestId,
    ),
    workerJobsUserIdx: index('worker_jobs_user_id_idx').on(table.userId),
    workerJobsTenantIdx: index('worker_jobs_tenant_id_idx').on(table.tenantId),
  }),
);
...
export const workerJobsRelations = relations(workerJobs, ({ one, many }) => ({
  parentJob: one(workerJobs, {
    fields: [workerJobs.parentJobId],
    references: [workerJobs.id],
    relationName: 'workerJobParent',
  }),
  childJobs: many(workerJobs, {
    relationName: 'workerJobParent',
  }),
  user: one(users, {
    fields: [workerJobs.userId],
    references: [users.id],
  }),
  tenant: one(tenants, {
    fields: [workerJobs.tenantId],
    references: [tenants.id],
  }),
}));
